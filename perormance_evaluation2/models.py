...
def load_or_train_model(model_name, model, X_train, y_train, X_test, y_test, 
                        task_type, models_dir, results_dir):
    model_file = os.path.join(models_dir, f"{model_name.replace(' ', '_').lower()}_{task_type}.pkl")
    results_file = os.path.join(results_dir, f"{model_name.replace(' ', '_').lower()}_{task_type}_metrics.pkl")
...
def train_and_evaluate(models, X_train, y_train, X_test, y_test, 
                       task_type='binary', models_dir="models", results_dir="results"):
    results = {}
    trained_models = {}
    ...
    for name, model in tqdm(models.items(), desc="Training models"):
        ...
        cv_file = os.path.join(results_dir, f"{name.replace(' ', '_').lower()}_{task_type}_cv.pkl")
